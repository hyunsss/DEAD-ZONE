...
namespace KINEMATION.FPSAnimationFramework.Runtime.Core
{
    public abstract class FPSAnimatorLayerState
    {
        ...
        protected float GetCurveBlendValue(CurveBlend blend)
        {
            ...
            {
                value = Mathf.Clamp01(_inputController.GetValue<float>(blend.name));
            }
            else
            {
                value = Mathf.Clamp01(_playablesController.GetCurveValue(blend.name,
                                        blend.source == ECurveSource.Animator));
            }
